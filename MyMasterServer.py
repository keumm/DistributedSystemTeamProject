...
class MasterServer:
    def __init__(self,statue,PORT):
        self.PORT = PORT
        self.statue = statue
        
    def serverRun(self):
        if self.statue == "Master":
            self.WhenServerIsOn()
        elif self.statue == "Shadow":
            self.WhenServerIsOff()
        else:
            print("Unknown Type Server")
...
# Need to put When server is ON or not
    ...
    def transfer(self,src, dst, direction):
        ...
        while True:
            try:
                ...
                if len(buffer) > 0:
                    dst.send(self.handle(buffer, direction, src_address,
    ...
    def server(self):
        port = self.ExtractHearbeatIntoList()
        ...
        while True:
            remote_port = self.ExtractHearbeatIntoList()
    ...
    def ExtractHearbeatIntoList(self):
        f = open("heartbeat.txt", "r")
        aList = f.read()
        aList = aList.replace('', '').split(" ")
        a_dict = {}
    ...
    def ExtractHeartbeatIntoFullList(self):
    ...
    def InternalSocketPart_ServerSide(self):
                ...
                if sock == server_socket:
                    ...
                    if data:
                        ...
                        if data.startswith(b'{'):
                            ...
                            with open(self.globaluserlist, 'w') as f:
                                json.dump(data, f)
                        ...
                        # If the data is start with nothing. (which means heartbeat data)
                            ...
                            self.update_hearbeat(data.decode())
                            sock.sendall(data)
                    else:
                        print(
    ...
    def SendDataToShadowMaster(self):
        server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    ...
    def ShadowMasterServer_ClientSide(self):
        print('Hi, I am a shadow Master')


    # Normal Master Mode
    #########################################################################################
    def WhenServerIsOn(self):
        t1 = threading.Thread(target=self.InternalSocketPart_ServerSide)
        t2 = threading.Thread(target=self.OpenWebpage)
        t3 = threading.Thread(target=self.server)
        t4 = threading.Thread(target=self.SendDataToShadowMaster)
    ...
    def ConnectingAndGetDataFromMaster(self):
    ...
    def ChangeTheMode(self):
        os.system('python master_shadow[test].py --verbose &')


    def RunChunkServerScript(self):
        ports = self.ExtractHeartbeatIntoFullList()
    ...
    # Shadow Master Mode
    ###############################################################################
    def WhenServerIsOff(self):

        t1 = threading.Thread(target=self.ConnectingAndGetDataFromMaster)
        t1.start()
    ################################################################################
...
def main():
    ...
    server = MasterServer("Master",5000)
    shadowServer = MasterServer("Shadow",54321)
    if args.verbose:
